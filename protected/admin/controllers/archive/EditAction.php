...
class EditAction extends BaseAction{
  ...
  protected function do_action(){ 
  	...
  	if(isset($_POST[$model_name])){
		  ...
		  $save_path="upload/archive/".$model->channel_id;
		  if(in_array('1',$addition_select)){
		   ...
		   if(!$select_image){	
			 ...
			 $select_upload_file=$_REQUEST['select_upload_file'];
		  	if(!$select_upload_file){
		     	$upload_file1=CUploadedFile::getInstance($model, 'upload_file');
		     	if(!empty($upload_file1->name)){
					  $model->upload_file=Util::rename_file($upload_file1->name);
			   	}
				}else{
			...
			if($model->validate()){			 
			  //上传图片
				if((!$select_image)&&($upload_file!=null)&&!empty($model->image)&&!in_array('1',$addition_select)){
			     ...
			     //上传图片
				if((!$select_upload_file)&&($upload_file1!=null)&&!empty($model->upload_file)){
					  
					  $save_path1="upload/archive/".$model->channel_id."/attachment";
			   	  Util::makeDirectory($save_path1);
				  	$upload_file1->saveAs($save_path1."/".$model->upload_file);
				  	$model->upload_file=$save_path1."/".$model->upload_file;
			  }
