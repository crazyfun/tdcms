<?php
class ScustomAction extends BaseAction{
	protected function beforeAction(){
        $this->controller->init_user_page();
        $this->controller->user_tag="scustom";
   ...
   protected function do_action(){
		...
		if(!empty($type)){
		   ...
		   $page_params['type']=$type;
			 ...
			 $service_category=ServiceCategory::model();
			 $child_datas =$service_category->get_descendant("29");
		   array_push($conditions," ServiceCustom.type ".Util::db_create_in($child_datas));
		}
  	 ...
  	 //生成ActiveDataProvider对象
		...
		$this->display("s_custom",array('model'=>$model,'dataProvider'=>$dataProvider,'page_params'=>$page_params,'cssPath'=>$cssPath,'jsPath'=>$jsPath));
  }
