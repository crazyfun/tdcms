...
class Region extends BaseActive
{
	...
	public function rules()
	{
		...
		return array(
		  array('region_name','required','message'=>'{attribute}不能为空','on'=>'AdminLogin'),
			array('sort_order,open', 'numerical', 'integerOnly'=>true),
			array('region_name', 'length', 'max'=>100),
			array('parent_id', 'length', 'max'=>10),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('region_id, region_name, parent_id, open,sort_order', 'safe', 'on'=>'search'),
		);
	}
    ...
    function get_select_regions(){
       ...
       foreach($charaters as $key => $value){
       	 ...
       	 $tem['name']=$value;
       	 $tem['children']=array();
         $children_datas=$this->findAll(array('select'=>'region_id,region_name,parent_id','condition'=>"sort_order=:sort_order AND open=:open",'params'=>array(':sort_order'=>$key,':open'=>'2')));
   ...
   //获取选择区域的选择项
    function get_options_regions(){
         $region_datas=$this->findAll(array('select'=>'region_id,region_name','condition'=>"open=:open",'params'=>array(':open'=>'2'),'order'=>'sort_order ASC'));
         $return_array=array();
