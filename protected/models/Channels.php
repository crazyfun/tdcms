...
class Channels extends BaseActive
{
	...
	public function rules()
	{
		...
		return array(
			array('name,permission,link_type,is_hidden,cover_template,lists_template,archive_template', 'required','message'=>'{attribute}不能为空'),
			...
			array('name,list_sort_type', 'length', 'max'=>30),
	...
	public function delete_table_datas($pk_id="",$condition=array()){
      ...
      foreach($child_menu_datas as $key1 => $value1){
    	  ...
    	  $archives->updateAll(array('status'=>'3'),"channel_id=:channel_id",array(':channel_id'=>$child_menu_id));
      }
			$this->deleteByPk($id,"",array());
			$archives->updateAll(array('status'=>'3'),"channel_id=:channel_id",array(':channel_id'=>$id));
		}
	 }else{
	 	  $id=$table_datas->id;
			$child_menu_datas=$this->findAll(array('select'=>'id','condition'=>'parent_id=:parent_id','params'=>array(':parent_id'=>$id)));
	...
	function get_channel_archives($channel_id,$category_id="",$attr="",$order="t.update_time DESC"){
		$archives=Archives::model();
		$conditions=array();
		$params=array();
		if(empty($channel_id)){
		...
		$archives_data=$archives->with("Channels")->findAll(array('condition'=>implode(" AND ",$conditions),'params'=>$params,'order'=>$order));
		return $archives_data;
	}
	
	function get_move_select($parent_id,$level=0){
		$channels_select=array();
		if($parent_id=="0"){
			$channels_select[0]="根栏目";
		}
		$child_datas=$this->get_child_channels($parent_id);
		$level++;
		foreach($child_datas as $key => $value){
		  ...
		  $tem_child_datas=$this->get_move_select($value->id,$level);
	...
	function get_channel_select($parent_id,$level=0){
		$channels_select=array();
		...
		foreach($child_datas as $key => $value){
		  ...
		  $name.=$value->name;	
		  $channels_select[$value->id]=$name;
		  $tem_child_datas=$this->get_channel_select($value->id,$level);
		  if(!empty($tem_child_datas)){
		  	$channels_select=$channels_select+$tem_child_datas;
		  }
		}
	...
	function get_child_channels($parent_id,$show='',$ids=""){
		$conditions=array();
		$params=array();
		if(!empty($show)){
...
//获得搜索时的栏目
 function get_search_channel($parent_id,$level=0,$ids=""){
 	  if(empty($ids)){
 	  	$tem_ids=array();
 	  	$archives=Archives::model();
 	  	$channel_datas=$this->findAll(array('select'=>'t.id,t.parent_id','condition'=>"t.is_hidden=:is_hidden",'params'=>array(':is_hidden'=>'1')));
 	  	foreach($channel_datas as $key => $value){
 	  		$archives_numbers=$archives->count("channel_id=:channel_id",array(':channel_id'=>$value->id));
 	  		if($archives_numbers){
 	  			$tem_ids[]=$value->id;
 		...
 		$channels_select=array();
		$child_datas=$this->get_child_channels($parent_id,1,$ids);
		$level++;
		foreach($child_datas as $key => $value){
		  $name="";
		  for($ii=1;$ii<$level;$ii++){
		  ...
		  $name.=$value->name;	
		  $channels_select[$value->id]=$name;
		  $tem_child_datas=$this->get_search_channel($value->id,$level,$ids);
		  if(!empty($tem_child_datas)){
		  	$channels_select=$channels_select+$tem_child_datas;
		  }
		}
  ...
  function get_channel_menus($parent_id,$show='',$ids=""){
		...
		foreach((array)$attributes as $key => $value){
		  $tem_child_datas=$this->get_channel_menus($value['id'],$show,$ids);
		
		  $have_children=$this->get_child_channels($value['id'],$show,$ids);
		  $value['link_href']=$this->set_channel_link($value['id']);
		  if(!empty($tem_child_datas)){
	...
	function set_channel_link($channel_id){
		...
		if($channel_data->link_type=='3'){
		   ...
		   if($channel_data->link_type=='1'){
		   	     return Yii::app()->createUrl("mchannels/list",array('channel'=>$channel_data['id']));
    	      
    	  }
    	  if($channel_data->link_type=='2'){
    	  	  return Yii::app()->createUrl("mchannels/index",array('channel'=>$channel_data['id']));
    	  }
		}
	}
