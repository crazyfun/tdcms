 ...
 */
class ServiceCategory extends BaseActive
{
	/**
	...
	public function relations()
	{
		...
		return array(
			'Parent'=>array(self::BELONGS_TO, 'ServiceCategory', 'parent_id'),
		);
	}
  ...
  //删除一笔数据
	public function delete_table_datas($pk_id="",$condition=array()){
		...
		$service_custom=new ServiceCustom();
		if(is_array($table_datas)){
		foreach($table_datas as $key => $value){
			$id=$value->id;
			$service_datas=$service_custom->findAll(array('select'=>'id','condition'=>'type=:type_id','params'=>array(':type_id'=>$id)));
      foreach($service_datas as $key1 => $value1){
    	  $service_id=$value1->id;
    	  $service_custom->delete_table_datas($service_id,array());
      }
 ...
 function get_select($parent_id,$level=0){
	...
	function get_category($parent_id,$ids=""){
		...
		foreach((array)$attributes as $key => $value){
		  $tem_child_datas=$this->get_category($value['id'],$ids);
		  if(!empty($tem_child_datas)){
