...
class User extends BaseActive{
	...
	public function rules()
	{
      ...
      array('var_user_password', 'compare', 'compareAttribute'=>'user_password','message'=>'两次输入的密码不一致','on'=>'WebRegiste'),
	    array('agreement', 'compare', 'compareValue'=>'1','message'=>'未同意立火旅行网会员协议','on'=>'WebRegiste'),
      array('user_login,real_name,user_email,user_password','required','message'=>'{attribute}不能为空','on'=>'UcenterRegiste'),
      ...
      array('user_login','length','max'=>30),
			array('admin_status,genter', 'length', 'max'=>1),
			array('user_password,permissions,invite_code', 'length', 'max'=>100),
			...
			array('station_id,credits,conpon','length','max'=>11),
			array('create_time,create_id', 'length', 'max'=>11),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id,user_email,user_password,user_salt,admin_status,real_name,address,create_id,cell_phone,create_time,avater,genter,credits,conpon,birthday,code,invite_code', 'safe', 'on'=>'search'),
		);
	}
