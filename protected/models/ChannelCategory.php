...
class ChannelCategory extends BaseActive
{
 ...
 function get_select($parent_id,$level=0){
	...
	function get_category($parent_id,$channel="",$ids=""){
		...
		foreach((array)$attributes as $key => $value){
		  $tem_child_datas=$this->get_category($value['id'],$channel,$ids);
		  if(!empty($tem_child_datas)){
		  	$value['children']=$tem_child_datas;
		  }
		  $value['href']=Yii::app()->getController()->createUrl("mchannels/list",array('channel'=>$channel,'category'=>$value['id']));
