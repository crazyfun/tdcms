...
class CouponResume extends BaseActive
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id,type,value','required','message'=>'{attribute}不能为空'),
			array('type', 'numerical', 'integerOnly'=>true),
			array('user_id, value, create_time', 'length', 'max'=>11),
			array('comment', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, user_id, type, value, comment, create_time', 'safe', 'on'=>'search'),
		);
	}
